...
function PopupPage() {
  ...
  var AddNewProfile = function() {
    ...
    profile.IsAutoLogin =
      $("#AddUserAutoLogin").val() == "on" && !IsNullOrEmpty(profile.Password);
    ...
    for (var index in existingProfiles) {
      if (
        existingProfiles[index].ServerAddress == profile.ServerAddress &&
  ...
  var RefreshProfiles = function() {
    ...
    $(".Profile").click(function(e) {
      if (!$(e.target).hasClass("DeleteButton")) {
        ...
        var server = $(".ProfileDetails.Server", this)
          .first()
          .text();
        var uid = $(".ProfileDetails.UID", this)
          .first()
          .text();
  ...
  var RemoveProfile = function(server, uid, username) {
    ...
    console.log(
      "Profile remove requested for [" + server + "](" + uid + ")",
      "PopupPage"
    );
    ...
    for (var index = 0, cache = profiles.length; index < cache; index++) {
      ...
      if (
        (server.indexOf(profile.ServerAddress) !== -1 &&
  ...
  var GetProfileView = function(profile) {
    ...
    var deleteButton = $("<div>")
      .addClass("ProfileDetails DeleteButton")
      .append("r");
    var profileImage = $("<img>")
      .addClass("ProfileDetails Image")
      .attr({
        src:
          profile.UID == "unknown"
    ...
    var title = $("<div>")
      .append(
        $("<div>")
          .addClass("ProfileDetails UserName")
          .append(profile.Name)
      )
      .append(
        $("<div>")
          .addClass("ProfileDetails UID")
          .append(profile.UID)
          .attr("title", _gim("popup_ProfileItem_TitleUID"))
      );
    ...
    var villages = $("<div>")
      .addClass("ProfileDetails Village")
      .append(
        $("<div>")
          .addClass("ProfileDetails")
          .append(
            profile.Villages.length + " " + _gim("popup_ProfileItem_Villages")
          )
      )
      .append(
        $("<div>")
          .addClass("ProfileDetails")
          .append(totalPopulation + " " + _gim("popup_ProfileItem_Population"))
      );
    var serverDetails = $("<div>")
      .addClass("ProfileDetails Server")
      .append(profile.ServerAddress);
    var rightContainer = $("<div>")
      .addClass("RightContainer")
      .append(title)
      .append(villages)
  ...
  var ExtensionActive = function(state) {
    var settings =
      JSON.parse(localStorage.getItem("Settings")) || new Models.OptionsModel();
