...
function ResourceCalculator() {
  ...
  var BuildCostCalculator = function(colors, times) {
    ...
    for (var rindex = 0; rindex < 5; rindex++) {
      ...
      $(".contractCosts, .information, .regenerateCosts").each(function(
        cindex
      ) {
        $(".showCosts span:eq(" + rindex + ")", this).each(function() {
          ...
          DLog(
            "ResourceCalculator - Appended cost element for resource [r" +
              (rindex + 1) +
              "] difference [" +
              diff +
              "]"
          ...
          DLog(
            "Appended time deference element for resource [r" +
              (rindex + 1) +
              "] difference [" +
              timeDifference +
              "]",
  ...
  var RefreshBuildFunction = function(colors, times) {
    ...
    $(".ResourceCalculatorBuildFillTime").each(function() {
      ...
      if (secondsLeft == 0) $(this).css("color", colors.ColorZero);
  ...
  var UnitCostCalculator = function(colors) {
    ...
    $.each(costs, function(iindex) {
      ...
      for (var rindex = 0; rindex < 5; rindex++) {
        ...
        if (rindex > 3) {
          $("span:eq(" + rindex + ")", costs[iindex]).append(
            $("<div>")
              .append("empty")
              .hide()
          );
  ...
  var RefreshUnitsFunction = function(input, cost, colors) {
    ...
    for (var rindex = 0; rindex < 4; rindex++) {
      var inWarehouse =
        ActiveProfile.Villages[ActiveVillageIndex].Resources.Stored[rindex];

      // Get resource cost of one unit
      var resourceCost =
        parseInt($("span:eq(" + rindex + ")", cost).text(), 10) || 0;
...
var ResourceCalculatorMetadata = {
  ...
  Version: "0.2.5.0",
  Description:
    "Shows you how much of each resource is needed to build field, building or train army. ",
