...
function MarketplaceEnhancements() {
  ...
  this.Register = function() {
    ...
    Log(
      "Registering MarketplaceEnhancements plugin...",
    ...
    if (ActivePageTravianVersion === "4" || ActivePageTravianVersion == "4.2") {
      ...
      tradersAvailable =
        escape($(".merchantsAvailable").html()).split(/[A-Z]|%/)[4] || 0;
    }

    DLog(
      "Trader Max Transport: " + traderCarryAmount,
      "MarketplaceEnhancements"
    );
    ...
    var useHour = $("<span>")
      ...
      .click(function() {
        $.each($("#send_select input"), function(index, obj) {
          var production =
            ActiveProfile.Villages[ActiveVillageIndex].Resources.Production[
              index
            ];
  ...
  var AddFullButton = function() {
    var fill = $("<span>")
      ...
      .click(function() {
        var VillageIndex = document.getElementById("enterVillageName_list")
          .value;
        var villageStorage =
          ActiveProfile.Villages[VillageIndex].Resources.Stored;
        var villageMaxStorage =
          ActiveProfile.Villages[VillageIndex].Resources.Storage;
  ...
  var ValidateHourButton = function() {
    ...
    for (var index = 0; index < 4; index++) {
      ...
      if (stored < production + alreadySet) {
        DLog(
          "Not enough " +
            Enums.FieldNames[index] +
            " stored [" +
            stored +
            "] to send " +
            (alreadySet ? "more than" : "hour production") +
            " [" +
            (production + alreadySet) +
            "]",
          "MarketplaceEnhancements"
        );
        ...
        hourButton.attr(
          "title",
          hourButton.attr("title") +
            "\nNot enough " +
            Enums.FieldNames[index] +
            " - " +
            (production + alreadySet - stored) +
            " more needed"
        );
      }
    ...
    if (canTransport < productionSum) {
      DLog(
        "Not enough traders to send [" + productionSum + "] resources",
        "MarketplaceEnhancements"
      );
      hourButton.button("option", "disabled", true);
      hourButton.attr(
        "title",
        hourButton.attr("title") +
          "\nNot enough traders available - " +
          (Math.ceil(productionSum / traderCarryAmount) - tradersAvailable) +
          " more needed"
      );
    }
  };
  ...
  var AddSpiners = function() {
    ...
    $("#r1, #r2, #r3, #r4").each(function(index, obj) {
      ...
      var stored =
        ActiveProfile.Villages[ActiveVillageIndex].Resources.Stored[index];
      ...
      $(obj)
        .spinner({
          ...
          spin: function(event, ui) {
            ...
            if (
              (isInc && ui.value - 1 + traderCarryAmount <= stored) ||
            ...
            $(this).spinner(
              "value",
              ui.value + spinStep * stepValue - stepValue
            );
            $(event.target).change();
            return false;
          }
  ...
  var InsertJunkResourceTable = function() {
    ...
    var junkResourcesTable = $("<table>")
      .addClass("PAMEJunkTable")
      .attr({
        cellpadding: 1,
        cellspacing: 1
      });
    ...
    merchantsRow.append(
      $("<td>")
        .addClass("tradersNeeded")
        .append("0")
    );
    merchantsRow.append(
      $("<td>")
        .addClass("tradersAvailable")
        .append("/ " + tradersAvailable)
    );
    ...
    wastedRow.append(
      $("<td>").append(
        $("<div>")
          .append(
            $("<img>")
              .addClass("r1 PAMEJunkTableResourceLink Disabled")
              .attr({ src: "img/x.gif", title: "Add waste to Lumber" })
              .click(UseJunkResource)
          )
          .append(
            $("<img>")
              .addClass("r2 PAMEJunkTableResourceLink Disabled")
              .attr({ src: "img/x.gif", title: "Add waste to Clay" })
              .click(UseJunkResource)
          )
          .append(
            $("<img>")
              .addClass("r3 PAMEJunkTableResourceLink Disabled")
              .attr({ src: "img/x.gif", title: "Add waste to Iron" })
              .click(UseJunkResource)
          )
          .append(
            $("<img>")
              .addClass("r4 PAMEJunkTableResourceLink Disabled")
              .attr({ src: "img/x.gif", title: "Add waste to Crop" })
              .click(UseJunkResource)
          )
      )
    );
    ...
    $(".send_res tbody").append(
      $("<tr>").append(
        $("<td>")
          .attr("colspan", "4")
          .append(junkResourcesTable)
      )
    );
    ...
    Log(
      "Junk resources table inserted successfully...",
      "MarketplaceEnhancements"
    );
  };
  ...
  var UseJunkResource = function() {
    ...
    var stored =
      ActiveProfile.Villages[ActiveVillageIndex].Resources.Stored[
        targetResourceIndex - 1
      ];
    var junkAmount = parseInt($(".junkAmount").text(), 10) || 0;

    targetElement.spinner(
      "value",
      resourceReserved + Math.min(junkAmount, stored - resourceReserved)
    );
  ...
  var FillInJunkResourceTable = function() {
    ...
    if (tradersNeeded > tradersAvailable)
      $(".tradersNeeded")
        .parent()
        .addClass("PAMEJunkTableErrorRow");
    else
      $(".tradersNeeded")
        .parent()
        .removeClass("PAMEJunkTableErrorRow");
    ...
    if (junkAmount !== 0) {
      ...
      for (var index = 0; index < 4; index++) {
        ...
        if (stored - resourceReserved > 0)
          $(".PAMEJunkTableResourceLink:eq(" + index + ")").removeClass(
            "Disabled"
          );
      }
    } else
  ...
  var IncomingSum = function() {
    // TODO Comment
    Log(
      "Generating table for incoming resources sum...",
      "MarketplaceEnhancements"
    );
    ...
    var CrawlMarketplaceTables = function() {
      ...
      if (groups === 2) {
        ...
        arrivingTables = $(
          "#merchantsOnTheWayFormular h4:first:first-of-type"
        ).nextUntil("h4");
      } else if (groups === 1) {
      ...
      var resourceSplit = $(".res > td > span", arrivingTables)
        .text()
        .split(" ");
      for (
        var index = 1, cache = resourceSplit.length;
        index < cache;
        index++
      ) {
...
var MarketplaceEnhancementsMetadata = {
  ...
  Version: "0.1.0.0",
  Description:
    "Adds number of improvements to the marketplace, quick village selection, resource shortcuts, junk resource links and many more.",
...
GlobalPluginsList[GlobalPluginsList.length] = $.extend(
  ...
  MarketplaceEnhancementsMetadata
);
