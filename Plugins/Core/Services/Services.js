...
function Services() {
  ...
  var CrawlPage = function() {
    ...
    if (
      ...
      if (
        MatchPages([Enums.TravianPages.Player]) &&
        (!URLContains("uid") || URLContains(ActiveProfile.UID))
      ) {
  ...
  var CrawlVersion = function() {
    ...
    if (!IsLogedIn) {
      DLog(
        "Can't crawl for travian version when player isn't loged in",
        "Services"
      );
      return;
    }
    ...
    else if ($("#sidebarBoxDailyquests").length)
      ActivePageTravianVersion = "4.4";
    else if ($(".sidebarBoxInnerBox .heroImage").length)
      ActivePageTravianVersion = "4.2";
  ...
  var CrawlStorage = function() {
    ...
    if (ActivePageTravianVersion === "4") {
      $("#res > li > p > span").each(function(index, obj) {
        ...
        if (index < 4) {
          ...
          activeVillage.Resources.Storage[index] =
            parseInt(separated[1], 10) || 0;

          // Set current stored value
          activeVillage.Resources.Stored[index] =
            parseInt(separated[0], 10) || 0;
        } else {
      ...
      for (var index = 0; index < 4; index++) {
        activeVillage.Resources.Stored[index] =
          $("#l" + (index + 1))
            ...
            .replace(".", "") || 0;
        activeVillage.Resources.Storage[index] =
          index == 3 ? granarySize : warehouseSize;
      }

      // Crawl Free Crop
      activeVillage.Resources.FreeCrop =
        $("#stockBarFreeCrop")
          .text()
          .replace(",", "")
          .replace(" ", "")
    ...
    DLog(
      "Stored in Village [" +
        activeVillage.VID +
        "] is [" +
        activeVillage.Resources.Stored +
        "] and free crop [" +
        activeVillage.Resources.FreeCrop +
        "]",
      "Services"
    ...
    DLog(
      "Storage of Village [" +
        activeVillage.VID +
        "] is [" +
        activeVillage.Resources.Storage +
        "]",
      "Services"
    );
  };
  ...
  var CrawlProduction = function() {
    ...
    if (m != null) {
      for (var index = 1; index < 5; index++) {
        activeVillage.Resources.Production[index - 1] =
          parseInt(m[index], 10) || 0;
      }
      ...
      DLog(
        "Production of Village [" +
          activeVillage.VID +
          "] is [" +
          activeVillage.Resources.Production +
          "]",
        "Services"
      );
  ...
  var CrawlLoyalty = function() {
    ...
    if (ActivePageTravianVersion === "4") {
      ...
      activeVillage.Loyalty =
        parseInt(
          $("#sidebarBoxActiveVillage div.loyalty.medium > span")
            .text()
            .replace(/[^\d]/g, ""),
          10
        ) || 0;
  ...
  var CrawlVillageList = function() {
    ...
    for (var index = 0; index < villagesFomList.length; index++) {
      ...
      DLog(
        'Found village "' + village.name + '" [' + village.id + "]",
        "Services"
      );
      ...
      for (
        var indexActive = 0, cache = ActiveProfile.Villages.length;
        indexActive < cache;
        indexActive++
      ) {
        ...
        if (obj.VID == village.id) {
          ...
          if (obj.IsActive) {
            ...
            DLog(
              "Set ActiveVilageIndex to [" + ActiveVillageIndex + "]",
              "Services"
            );
          }
    ...
    if (!mainVillageFound) {
      // TODO Localize
      $("#sidebarBoxVillagelist .innerBox.content").append(
        "<br/><div>Capital village not detected. Visit your <a href='/spieler.php' style='color: #00BC00; text-decoration: underline;'>profile page</a>.</div>"
      );
    }
  };
  ...
  var CrawlMessages = function() {
    ...
    var currentReportsCount =
      parseInt($(".messages .speechBubbleContent").text(), 10) || 0;
    ...
    if (MatchPages([Enums.TravianPages.Messages])) {
      ...
      if (
        $(".messages .container.active a[href*='t=0']").length ||
        ...
        if ($(".paper").length) {
          ...
          $(".inbox > tbody > tr").each(function() {
            ...
            message.MID = parseInt(
              messageLink.attr("href").replace("nachrichten.php?id=", "")
            ).toString();
            message.IsRead = $(".subject img", messageElement).hasClass(
              "messageStatusRead"
            );
    ...
    DLog(
      "Services: CrawlReports found [" + currentReportsCount + "] new messages"
    );
  };
  ...
  var CrawlHero = function() {
    ...
    var canAdventure = $(".heroStatusMessage .heroStatus100").length != 0;
    var hasAdventure =
      (parseInt($(".adventureWhite .speechBubbleContent").text(), 10) || 0) >=
      1;
  ...
  var CrawlReports = function() {
    ...
    var currentReportsCount =
      parseInt($(".reports .speechBubbleContent").text(), 10) || 0;
    ...
    DLog(
      "Services: CrawlReports found [" + currentReportsCount + "] new reports"
    );
  };
  ...
  var CrawlVillagesDetails = function() {
    ...
    $.each(ActiveProfile.Villages, function(index, obj) {
      ...
      var village = $("#villages tbody tr")
        .find("td:contains('" + obj.Name + "')")
        .parent();

      // Crawl IsMainCity
      var checkMainCity = $(".name", village).has(".mainVillage").length
        ? true
        : false;
      ...
      var PositionX = $(".coordinateX", village)
        .text()
        .replace(/\D/g, "")
        .trim();
      ...
      var PositionY = $(".coordinateY", village)
        .text()
        .replace(/\D/g, "")
        .trim();
  ...
  var ProcessProfiles = function() {
    ...
    if (!IsLogedIn) LazyMatchExistingProfile();
  ...
  var MatchExistingProfile = function() {
    ...
    try {
      ...
      var matchedProfilesIndexes = GetMatchingProfilesIndexes(
        activeProfileUID,
        activeProfileName,
        ActiveServerAddress
      );
      ...
      if (matchedProfilesIndexes.length >= 1) {
        ...
        if (matchedProfilesIndexes.length != 1) {
          Warn(
            "Invalid number of profiles found that match current player [" +
              matchedProfilesIndexes.length +
              "]. Can't select profile automatically - selecting FIRST.",
            "Services"
          );
        }
      }
      ...
      Warn(
        "Couldn't retireve profile specific values. Creating new profile. Reason: ",
        ex
      );
    }
  ...
  var UpdateActiveProfile = function(uid, name, tribeID) {
    ActiveProfile = IsNullOrEmpty(ActiveProfile)
      ? new Models.Profile()
      : ActiveProfile;
  ...
  var GetMatchingProfilesIndexes = function(uid, name, server) {
    ...
    for (
      var index = 0, cache = AvailableProfiles.length;
      index < cache;
      index++
    ) {
      ...
      if (
        currentProfile.ServerAddress == server &&
  ...
  var LazyMatchExistingProfile = function() {
    ...
    if (matchedProfilesIndexes.length != 1) {
      Warn(
        "Invalid number of profiles found for current server [" +
          matchedProfilesIndexes.length +
          "]. Can't select profile automatically.",
        "Services"
      );
      return false;
    }
  ...
  var GetActiveProfileUID = function() {
    ...
    if (ActivePageTravianVersion === "4") {
      ...
      return (
        parseInt(profileLinkElement.replace("spieler.php?uid=", ""), 10) || 0
      );
    } else if (
      ActivePageTravianVersion === "4.2" ||
      ActivePageTravianVersion === "4.4"
      ...
      return (
        parseInt(imageSource.substr(start + 4, end - (start + 4)), 10) || 0
      );
    } else throw "Unsuported travian version";
  };
  ...
  var GetActiveProfileName = function() {
    ...
    if (ActivePageTravianVersion === "4") {
      return $(".sideInfoPlayer .signLink span")
        .text()
        .trim();
    } else if (
      ActivePageTravianVersion === "4.2" ||
      ...
      return $(".sidebarBoxInnerBox .playerName")
        .text()
        .trim();
    } else throw "Unsuported travian version";
  };
  ...
  var GetActiveProfileTribe = function() {
    ...
    if (ActivePageTravianVersion === "4") {
      return $(".sideInfoPlayer img")
        .attr("class")
        .replace("nationBig nationBig", "");
    } else if (
      ActivePageTravianVersion === "4.2" ||
      ActivePageTravianVersion === "4.4"
    ) {
      return $(".sidebarBoxInnerBox .playerName img")
        .attr("class")
  ...
  var SaveProfileChanges = function() {
    ...
    for (
      ...
      if (
        obj.ServerAddress == ActiveProfile.ServerAddress &&
    ...
    new Request("Background", "Data", "Profiles", {
      Type: "set",
...
var ServicesMetadata = {
  ...
  Version: "0.0.3.1",
  Description:
    "Takes care of all data retrieving and is requeired for all plugins",
...
GlobalPluginsList.push(
  $.extend(true, {}, Models.PluginMetadata, ServicesMetadata)
);
